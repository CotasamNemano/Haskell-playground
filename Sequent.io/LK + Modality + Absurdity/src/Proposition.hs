...
instance ShowLatex a => ShowLatex (Proposition a) where
    ...
    showLatexNonTop Top = "\\top"
    showLatexNonTop Bottom = "\\bot"
    ...
    showLatex (World a b) = showLatex a ++ "\\Vdash " ++ showLatex b
...
instance PropositionProperty (Proposition a) where
    ...
    haveModality (World _ MTop) = False
    haveModality (World _ MBottom) = False
    ...
    isClassical (World _ MTop) = True
    isClassical (World _ MBottom) = True
