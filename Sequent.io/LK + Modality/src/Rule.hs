...
rules = [
    identity, notL, notR, andL, andR, orL, orR, implyL, implyR, forallL, forallR, existsL, existsR,
    mNotR, mNotL, mAndR, mAndL, mOrR, mOrL, mImplyR, mImplyL, mBoxR, mBoxL, mDiamondR, mDiamondL, shuffleL, shuffleR, shuffleVar, exchangeL, exchangeR
    ]
...
notL (Open s@(Sequent var (Not p : gamma) delta)) =
    Just $ Rule1 "notL" s (Open $ Sequent var gamma (p : delta))
notL _ = Nothing

notR :: Rule a
notR (Open s@(Sequent var gamma (Not p : delta))) =
    Just $ Rule1 "notR" s (Open $ Sequent var (p : gamma) delta)
notR _ = Nothing
...
existsL (Open s@(Sequent v (Exists x p : gamma) delta)) =
    Just $ Rule1 "forallR" s (Open $ Sequent (newVariable : v) (substitute x newVariable p : gamma) delta) where
        newVariable = newVar v
existsL _ = Nothing
...
mDiamondR (Open s@(Sequent v gamma (World x (MBox p) : delta))) =
...
mDiamondL (Open s@(Sequent v gamma (World x (MBox p) : delta))) =
